...
class HalAnaFile : public TObject {
  ...
  void PrintCut(HalPackage* pack) const;
  void PrintCutsInPackage(HalPackage* pack) const;
  HalPackage* GetCutCollection(Hal::ECutUpdate update, Int_t no) const;
  HalPackage* GetMainPackage() const;
  HalPackage* GetCutContainer() const;
...
public:
   ...
   */
  HalAnaFile(TString filename, TString packname);
   ...
   */
  HalAnaFile(TString filename = " ", Int_t id = -1);
  /**
   *
   ...
   */
  HalPackage* GetPackage(TString name) const;
  /**
   *
   * @return global data (or local if global wasn't found)
   */
  HalPackage* GetMetaData() const;
  /**
   ...
   */
  HalPackage* GetCutPackage(Hal::ECutUpdate update, Int_t collection, Int_t cutNo, Option_t* type = "all") const;
  virtual ~HalAnaFile();
  ClassDef(HalAnaFile, 1)
};
