...
HalSpline1D::HalSpline1D(TH1* h, Double_t begval, Double_t endval) :
...
Double_t HalSpline1D::Eval(Double_t x) const {
  Int_t bin = fXaxis->FindBin(x);
  return fA[bin] + fB[bin] * x + fC[bin] * x * x;
}

Double_t HalSpline1D::GetError(Double_t x) const {
  Int_t bin = fXaxis->FindBin(x);
  return fAe[bin];
}

Double_t HalSpline1D::EvalBin(Int_t bin) const {
  Double_t x = fXaxis->GetBinCenter(bin);
  return fA[bin] + fB[bin] * x + fC[bin] * x * x;
}

Double_t HalSpline1D::ErrorBin(Int_t bin) const { return fAe[bin]; }

void HalSpline1D::Eval(Double_t x, Double_t& f, Double_t& error) const {
  Int_t bin = fXaxis->FindBin(x);
  f         = fA[bin] + fB[bin] * x + fC[bin] * x * x;
  error     = fAe[bin];
}

void HalSpline1D::CalcParams(Double_t x1,
                              Double_t y1,
                              Double_t x2,
                              Double_t y2,
                              Double_t x3,
...
void HalSpline1D::SetFirstBin(TH1* h, Double_t begval, Bool_t use_begval) {
...
void HalSpline1D::SetLastBin(TH1* h, Double_t endval, Bool_t use_endval) {
...
TH1D* HalSpline1D::GetTHD(TString name) const {
...
HalSpline2D::HalSpline2D(TH2* h, Option_t* opt_int) : fXaxis(NULL), fYaxis(NULL), fNbinsX(0), fNbinsY(0), fA(), fAe(NULL) {
  fXaxis  = (TAxis*) h->GetXaxis()->Clone("Xspline");
  fYaxis  = (TAxis*) h->GetYaxis()->Clone("Yspline");
  fNbinsX = fXaxis->GetNbins();
  fNbinsY = fYaxis->GetNbins();
  fAe     = new HalArray_2<Double_t>();
  ...
  if (fXaxis->IsVariableBinSize() && fYaxis->IsVariableBinSize()) {
    HalCout::PrintInfo("Bot axes have non-fixed bin size 2dim interpolation will probably not "
                        "work",
...
void HalSpline2D::Refit() {
  Double_t Params[9];
  HalArray_3<Double_t> tempA(fXaxis->GetNbins() + 2, fYaxis->GetNbins() + 2, 9);
  for (int i = 1; i <= fNbinsX; i++) {
    Double_t x[3];
...
Double_t HalSpline2D::Eval(Double_t x, Double_t y) const {
...
Double_t HalSpline2D::EvalBin(Int_t ix, Int_t iy) const {
...
Double_t HalSpline2D::ErrorBin(Int_t bin_x, Int_t bin_y) const { return fAe->Get(bin_x, bin_y); }

void HalSpline2D::Eval(Double_t x, Double_t y, Double_t& f, Double_t& error) const {
  Int_t ix    = fXaxis->FindFixBin(x);
  Int_t iy    = fYaxis->FindFixBin(y);
  Double_t x2 = x * x;
  Double_t y2 = y * y;
...
void HalSpline2D::CalcParams(Double_t x[3], Double_t y[3], Double_t z[3][3], Double_t params[9]) {
  TMatrixD AM(9, 9);
  TMatrixD BM(9, 1);
  for (int i = 0; i < 9; i++) {
    Int_t xi = i % 3;
...
void HalSpline2D::PrintParams(Double_t x, Double_t y) {
...
void HalSpline2D::Extrapolate(TH2* h, Option_t* interpolation) const {
...
Double_t HalSpline2D::GetExtrapolationParam(Double_t x, Double_t y, Int_t param) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
  return fA.Get(ix, iy, param);
}

Double_t HalSpline2D::Extrapolate(TH2* h, Int_t ix1, Int_t ix2, Int_t ix, Int_t iy1, Int_t iy2, Int_t iy, Int_t opt) const {
...
Double_t HalSpline2D::GetError(Double_t x, Double_t y) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
  return fAe->Get(ix, iy);
}

HalSpline2D::~HalSpline2D() {}

void HalSpline1D::FastOverwrite(TH1D* h, Double_t begval, Double_t endval) {
  SetFirstBin(h, begval, (Bool_t) begval);
  SetLastBin(h, endval, (Bool_t) endval);
  for (int i = 2; i < fXaxis->GetNbins(); i++) {
    Double_t x1 = h->GetBinCenter(i - 1);
...
void HalSpline2D::SetPoint(Int_t xbin, Int_t ybin, Double_t val, Double_t error) {
...
void HalSpline3D::CalcParams(Double_t x[2], Double_t y[2], Double_t z[2], Double_t v[2][2][2], Double_t params[8]) {
  TMatrixD AM(8, 8);
  TMatrixD BM(8, 1);
...
HalSpline3D::HalSpline3D(TH3* h, Option_t* interpolation) {
  Bool_t mid = kFALSE;
  fXaxis     = (TAxis*) h->GetXaxis()->Clone();
...
Double_t HalSpline3D::Eval(Double_t x, Double_t y, Double_t z) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
...
Double_t HalSpline3D::GetError(Double_t x, Double_t y, Double_t z) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
  Int_t iz = fZaxis->FindFixBin(z);
...
void HalSpline3D::PrintParams(Double_t x, Double_t y, Double_t z) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
  Int_t iz = fZaxis->FindFixBin(z);
  for (int i = 0; i < 8; i++)
...
void HalSpline3D::Eval(Double_t x, Double_t y, Double_t z, Double_t& f, Double_t& error) const {
  Int_t ix = fXaxis->FindFixBin(x);
  Int_t iy = fYaxis->FindFixBin(y);
  Int_t iz = fZaxis->FindFixBin(z);
...
Double_t HalSpline3D::EvalBin(Int_t ix, Int_t iy, Int_t iz) const {
  Double_t x = fXaxis->GetBinCenter(ix);
  Double_t y = fYaxis->GetBinCenter(iy);
...
Double_t HalSpline3D::ErrorBin(Int_t ix, Int_t iy, Int_t iz) const { return fAe.Get(ix, iy, iz); }

HalSpline3D::~HalSpline3D() {
  if (fXaxis) {
    delete fXaxis;
    delete fYaxis;
    delete fZaxis;
...
TH2D* HalSpline2D::GetTHD(TString name) const {
...
TH3D* HalSpline3D::GetTHD(TString name) const {
...
void HalSpline3D::Refit() {
  HalArray_4<Double_t> ANew;
...
Double_t HalSpline3D::EvalBin(Int_t ix, Int_t iy, Int_t iz, Double_t x, Double_t y, Double_t z) const {
  return fA.Get(ix, iy, iz, 0) + fA.Get(ix, iy, iz, 1) * x + fA.Get(ix, iy, iz, 2) * y + fA.Get(ix, iy, iz, 3) * z
         + fA.Get(ix, iy, iz, 4) * x * y + fA.Get(ix, iy, iz, 5) * x * z + fA.Get(ix, iy, iz, 6) * y * z
         + fA.Get(ix, iy, iz, 7) * x * y * z;
}

void HalSpline3D::FastOverwrite(TH3* h, Option_t* interpolation) {
  Bool_t mid       = kFALSE;
  TString opt      = interpolation;
  Int_t fix_method = 0;
  if (HalStd::FindParam(opt, "yes") || HalStd::FindParam(opt, "fix1")) { fix_method = 1; }
