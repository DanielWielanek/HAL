...
namespace HalStd {
  ...
  TH1D* GetProjection1D(const TH3* histo, Double_t min1, Double_t max1, Double_t min2, Double_t max2, Option_t* opt) {
    TString option = opt;
    Bool_t sumw    = HalStd::FindParam(option, "sumw", kTRUE);
  ...
  TH2D* GetProjection2D(const TH3* histo, Double_t min, Double_t max, Option_t* opt) {
    ...
    if (HalStd::FindParam(option, "yz")) {
      axis              = histo_cloned->GetXaxis();
      projection_option = "yze";
    } else if (HalStd::FindParam(option, "zy")) {
      axis              = histo_cloned->GetXaxis();
      projection_option = "zye";
    } else if (HalStd::FindParam(option, "xz")) {
      axis              = histo_cloned->GetYaxis();
      projection_option = "xze";
      // axisA = histo->GetXaxis();
      // axisB = histo->GetZaxis();
    } else if (HalStd::FindParam(option, "zx")) {
      axis              = histo_cloned->GetYaxis();
      projection_option = "zxe";
    } else if (HalStd::FindParam(option, "yx")) {
    ...
    if (HalStd::FindParam(option, "bins")) {
  ...
  TH2* MakeHisto2D(TString name, TString title, TVector3 Xaxis, TVector3 Yaxis, Char_t type) {
    switch (type) {
      case 'D': return new TH2D(name, title, (int) Xaxis.X(), Xaxis.Y(), Xaxis.Z(), (int) Yaxis.X(), Yaxis.Y(), Yaxis.Z()); break;
      case 'F': return new TH2F(name, title, (int) Xaxis.X(), Xaxis.Y(), Xaxis.Z(), (int) Yaxis.X(), Yaxis.Y(), Yaxis.Z()); break;
      case 'I': return new TH2I(name, title, (int) Xaxis.X(), Xaxis.Y(), Xaxis.Z(), (int) Yaxis.X(), Yaxis.Y(), Yaxis.Z()); break;
    }
    return nullptr;
  };
  ...
  void Fold2D(Double_t val, TH2& h, TString opt) {
    ...
    if (HalStd::FindParam(opt, "x", kTRUE)) {
      std::vector<int> foldBinMap = FoldAxis(*h.GetXaxis(), val);
    ...
    } else if (HalStd::FindParam(opt, "y", kTRUE)) {
  ...
  void Fold3D(Double_t val, TH2& h, TString opt) {
    ...
    } else if (HalStd::FindParam(opt, "z", kTRUE)) {
