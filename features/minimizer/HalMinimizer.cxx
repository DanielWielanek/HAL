...
HalMinimizer* HalMinimizer::fgInstance = NULL;
HalMinimizer::HalMinimizer() :
  fFunc(nullptr),
  fMinimizeType(kScan),
  fNCalls(0),
  fDiscreteFit(kFALSE),
...
bool HalMinimizer::SetFixedVariable(unsigned int ivar, const std::string& name, double val) {
...
bool HalMinimizer::SetVariableLimits(unsigned int ivar, double lower, double upper) {
  if (lower > upper) {
    double temp = upper;
    upper       = lower;
    lower       = upper;
...
bool HalMinimizer::SetLimitedVariable(unsigned int ivar,
...
void HalMinimizer::InitFit() {
  ...
  HalCout::Database(6, "ParName", "MinMap", "MaxMap", "Steps", "Min", "Max");
  for (int i = 0; i < fParameters.size(); i++) {
    if (fParameters[i].IsFixed()) {
      HalCout::Database(2, fParameters[i].GetParName().Data(), Form("%4.4f", fParameters[i].GetStartVal()));
    } else {
      HalCout::Database(6,
...
void HalMinimizer::Reset() { fParameters.clear(); }


// minimize methods


bool HalMinimizer::Minimize() {
...
const double* HalMinimizer::X() const {
  if (!fDiscreteFit) return fSmoothFits;
  return fQuantumFits;
}

void HalMinimizer::SetFunction(const ROOT::Math::IMultiGenFunction& func) {
...
Bool_t HalMinimizer::LoopOverParameter(Int_t param) {
  /** all parameters set **/
...
void HalMinimizer::EstimateErrors() {
  for (unsigned int i = 0; i < GetNParams(); i++) {
    if (IsFixed(i)) {
...
void HalMinimizer::EstimateError(Int_t par, Double_t& min, Double_t& quantumMin, Double_t& error) {
...
Bool_t HalMinimizer::IsFixed(Int_t i) const { return fParameters[i].IsFixed(); }

HalMinimizer* HalMinimizer::Instance() {
  if (fgInstance == NULL) fgInstance = new HalMinimizer();
  //	fgInstance->Reset();
  return fgInstance;
}

void HalMinimizer::FinishFit() {
  delete[] fTempParams;
  fTempParams = nullptr;
}

void HalMinimizer::SetMinimizerType(TString opt) {
  opt.ToLower();
  if (opt == "scan") {
    fMinimizeType = kScan;
  } else if (opt == "ant") {
...
void HalMinimizer::MinimizeScan() {
...
void HalMinimizer::ChangeStateVector(std::vector<Int_t>& vec) {
...
bool HalMinimizer::SetParLimits(unsigned int ivar, double min, double max) {
  if (min > max) {
    double temp = max;
    max         = min;
    min         = temp;
