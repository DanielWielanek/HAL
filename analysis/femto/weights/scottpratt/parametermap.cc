...
namespace Hal {
  namespace ScottPratt {
    ...
    bool CparameterMap::getB(string key, bool def) {
      ...
      if (itr != this->end()) {
        ...
        if (tf[0] == 't' || tf[0] == '1')
          ...
          sprintf(message,
                  "parameterMap::getB(), boolean parameter with key %s read in with value %s, set to false\n",
                  key.c_str(),
                  pstring.c_str());
          CLog::Info(message);
        }
    ...
    void CparameterMap::set(string key, vector<vector<double>> val) {
      ...
      for (int i = 0; i < static_cast<int>(val.size()); ++i) {
        ss << "    ";
        for (int j = 0; j < static_cast<int>(val[i].size()); ++j) {
          ss << val[i][j] << " ";
        }
        ss << "\n";
      }
    ...
    void CparameterMap::ReadParsFromFile(string filename) { ReadParsFromFile(filename.c_str()); }
