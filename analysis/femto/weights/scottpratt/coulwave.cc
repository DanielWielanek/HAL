...
namespace Hal {
  namespace ScottPratt {
    ...
    void CoulWave::GetFGPrimeArray(int Lmax,
                                   double x,
                                   double eta,
                                   vector<double>& FL,
                                   vector<double>& GL,
    ...
    void CoulWave::phaseshift_CoulombCorrect(int ell, double q, double eta, double& delta, double& ddeltadq) {
      ...
      if (fabs(eta) > 1.0E-10) {
        ...
        for (i = 0; i < ell; i++) {
          ...
          dgamowdq[i + 1] = dgamowdq[i] * (double((i + 1) * (i + 1)) + eta * eta) - (2.0 * eta * eta / q) * gamow[i];
        }
    ...
    void CoulWave::GetFGprime_ImagQ(int ellmax, double x, double eta, double* FL, double* GL, double* FLprime, double* GLprime) {
      ...
      F      = new double[ellmax + 1];
      G      = new double[ellmax + 1];
      Fprime = new double[ellmax + 1];
      Gprime = new double[ellmax + 1];
      double ff, root, sign;
      int n, l, nmax;
      double *A, *B;
    ...
    void CoulWave::GetFGprime_ComplexQ(int ell,
                                       complex<double> cx,
                                       complex<double> ceta,
                                       double* FL,
                                       double* GL,
    ...
    complex<double> CoulWave::cw2_small_r(int l, complex<double> r, complex<double> eta) {
      ...
      factor = pow(-1, double(2 * l + 1)) * pow(2.0 * rr, double(l + 1)) * exp(rr) / (cdcon1 * cdcon2);
      psi1   = -EULER;
      psi2   = -EULER;
      for (k = 1; k <= 2 * l + 1; k++) {
    ...
    CONVERGE1:
      ...
      for (k = 1; k <= 10000; k++) {
        fact1   = fact1 * (-2.0 * rr) * (double(l + k) + ceta) / (double(k) * double(2 * l + 1 + k));
