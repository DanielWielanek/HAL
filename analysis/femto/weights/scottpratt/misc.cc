...
namespace Hal {
  namespace ScottPratt {
    ...
    double Misc::DotProduct(FourVector& p, FourVector& q) { return p[0] * q[0] - p[1] * q[1] - p[2] * q[2] - p[3] * q[3]; }
    ...
    double Misc::triangle2(double m0sq, double m1sq, double m2sq) {
      ...
      answer = (m0sq * m0sq + m1sq * m1sq + m2sq * m2sq - 2.0 * (m0sq * m1sq + m0sq * m2sq + m1sq * m2sq)) / (4.0 * m0sq);
      return answer;
      return (m0sq * m0sq + m1sq * m1sq + m2sq * m2sq - 2.0 * (m0sq * m1sq + m0sq * m2sq + m1sq * m2sq)) / (4.0 * m0sq);
    }

    double Misc::GetRapidity(FourVector& pa) { return 0.5 * log((pa[0] + pa[3]) / (pa[0] - pa[3])); }

    double Misc::GetDely(FourVector& pa, FourVector& pb) { return GetRapidity(pa) - GetRapidity(pb); }
    ...
    complex<double> Misc::cexp(complex<double> z) { return exp(real(z)) * ceiphi(imag(z)); }

    complex<double> Misc::ceiphi(double phi) { return complex<double>(cos(phi), sin(phi)); }
    ...
    int Misc::iround(double x) { return int(floor(x + 0.5)); }
    ...
    double Misc::cgc(double j1, double m1, double j2, double m2, double J, double M) {
      ...
      return sign * sqrt(2.0 * J + 1)
             * gsl_sf_coupling_3j(lrint(2 * j1), lrint(2 * j2), lrint(2 * J), lrint(2 * m1), lrint(2 * m2), -lrint(2 * M));
    }
          ...
          if (j1 - m1 - z >= 0 && j - m - z >= 0 && j2 - j + m1 + z >= 0) {
            thesum = thesum
                     + pow(-1, z + j1 - m1) * cgc_fractorial(j1 + m1 + z, j1 - m1 - z)
                         * cgc_fractorial(j2 + j - m1 - z, j - m - z) / (cgc_factorial(z) * cgc_factorial(j2 - j + m1 + z));
          }
        }
    ...
    int Misc::cgc_delta(int x, int y) {
      if (x == y)
        return 1;
      else
        return 0;
    ...
    void Misc::Cubic(double a0, double a1, double a2, double a3, complex<double>& z1, complex<double>& z2, complex<double>& z3) {
    ...
    void Misc::CubicResolvant(double r, double s, double t, double x[], double& d) {
      ...
      if (d_new > delta) {
        ...
        double v0 = -h1 - h2;
        if (fabs(u0) == 0.0)
          u = Misc::signswitch(0.0, u0);
        else
          u = Misc::signswitch(pow(fabs(u0), r3), u0);
        if (fabs(v0) == 0.0)
          v = Misc::signswitch(0.0, v0);
        else
          v = Misc::signswitch(pow(fabs(v0), r3), v0);
        x[0] = u + v - h;
        x[1] = -r2 * (u + v) - h;
        x[2] = r4 * fabs(u - v);
        ...
        d = 0.0;
        if (fabs(h1) == 0.0)
          u = Misc::signswitch(0.0, -h1);
        else
          u = Misc::signswitch(pow(fabs(h1), r3), -h1);
        x[0] = u + u - h;
        x[1] = -u - h;
        x[2] = x[1];
        if (fabs(h1) <= eps) {
    ...
    bool Misc::file_exists(const std::string& file_name) { return file_exists(file_name.c_str()); }

    double Misc::mod(double x, double y) { return x - int(x / y) * y; }
    ...
    void Misc::outsidelong(FourVector& pa,
                           FourVector& pb,
                           double& qinv,
                           double& qout,
                           double& qside,
                           ...
                           double& deleta,
                           double& dely,
                           double& delphi) {  // qout is in pair frame
      ...
      if (delphi > 180.0) delphi = 360.0 - delphi;
    }

    void Misc::outsidelong_lcms(FourVector& pa,
                                FourVector& pb,
                                double& qinv,
                                double& qout,
                                double& qout_lcms,
