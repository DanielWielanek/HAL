...
void HalFitParam::SetRange(Double_t min, Double_t max) {
  if (min > max) {
    Double_t temp = max;
    max           = min;
    min           = temp;
...
void HalFitParam::SetMapRange(Double_t min, Double_t max, Int_t points) {
  if (points <= 1)
    SetIsFixed(kTRUE);
  else
    SetIsFixed(kFALSE);
...
void HalFitParam::Print(Option_t* option) const {
