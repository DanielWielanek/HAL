 ...
 */
#include "HalHelixBase.h"

#include <TMath.h>
#include <iostream>

Double_t HalHelixBase::fgHelixBz = 0.5;

HalHelixBase::~HalHelixBase() {}
...
HalHelixBase::HalHelixBase(const HalHelixBase& t) :
  TObject(t),
  fX0(t.fX0),
  fY0(t.fY0),
  fZ0(t.fZ0),
...
HalHelixBase::HalHelixBase(const TVector3& x, const TVector3& mom, Double_t charge, Double_t conversion) : HalHelixBase() {
  Double_t pt = mom.Pt();
...
Int_t HalHelixBase::BaseIntersection(Double_t R, TVector3& x1, TVector3& x2) const {
  Double_t s1, s2;
...
Double_t HalHelixBase::GetPeriod() const {
  if (fCurv == 0) {
    return 1E+9;
  } else {  //
...
void HalHelixBase::BasePrint() const {
...
void HalHelixBase::BaseSetParams(const TVector3& x, const TVector3& mom, Double_t charge, Double_t conversion) {
  Double_t pt = mom.Pt();
...
void HalHelixBase::BaseSetParams(Double_t x,
                                  Double_t y,
                                  Double_t z,
                                  Double_t px,
                                  Double_t py,
...
void HalHelixBase::SetMagField(Double_t Bz) { fgHelixBz = Bz; }

Double_t HalHelixBase::GetBz() { return fgHelixBz; }

HalHelixBase& HalHelixBase::operator=(const HalHelixBase& helix) {
  if (this != &helix) {
    fY0     = helix.fY0;
    fZ0     = helix.fZ0;
    fX0     = helix.fX0;
...
TVector3 HalHelixBase::BasePosition(Double_t s) const {
...
TVector3 HalHelixBase::BaseGetStartPoint() const { return TVector3(fX0, fY0, fZ0); }

Double_t HalHelixBase::FudgePathLenght(const TVector3& vec) const {
  Double_t s;
...
void HalHelixBase::BasePathLength(Double_t r, Double_t& s1, Double_t& s2) const {
  //
...
void HalHelixBase::BasePathLength(Double_t r, Double_t x, Double_t y, Double_t& s1, Double_t& s2) const {
  HalHelixBase helix(*this);
  helix.fX0 = fX0 - x;
  helix.fY0 = fY0 - y;
...
Double_t HalHelixBase::BasePathLength(const TVector3& point, Bool_t scanPeriods) const {
...
Double_t HalHelixBase::BasePathLength(Double_t x, Double_t y) const { return FudgePathLenght(TVector3(x, y, 0)); }

void HalHelixBase::PathLengths(const HalHelixBase& h, Double_t& s1, Double_t& s2) const {
...
Double_t HalHelixBase::Distance(const TVector3& p, Bool_t scanPeriods) const {
  return (BasePosition(BasePathLength(p, scanPeriods)) - p).Mag();
}

HalHelixBase::HalHelixBase(Double_t x,
                             Double_t y,
                             Double_t z,
                             Double_t px,
                             Double_t py,
                             ...
                             Double_t charge,
                             Double_t conversion) :
  HalHelixBase(TVector3(x, y, z), TVector3(px, py, pz), charge, conversion) {}

void HalHelixBase::BaseShift(Double_t x, Double_t y, Double_t z) {
  fXc += x;
  fYc += y;
  fX0 += x;
  fY0 += y;
...
Double_t HalHelixBase::GetRotationDirection() const { return TMath::Sign(1, GetH() * TMath::Cos(GetDipAngle())); }

TVector3 HalHelixBase::BaseMomentum(Double_t s) const {
...
Double_t HalHelixBase::GetPathAbsMin(Double_t s1, Double_t s2) const {
...
void HalHelixBase::BaseFullEval(Double_t s, TVector3& mom, TVector3& pos) const {
  pos = BasePosition(s);
  if (fCurv != 0) {
