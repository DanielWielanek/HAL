...
#include <cstdlib>
namespace Hal {
  double* FemtoYlm::fgPrefactors         = 0x0;
  int* FemtoYlm::fgPrefshift             = 0x0;
  int* FemtoYlm::fgPlmshift              = 0x0;
  FemtoYlm* FemtoYlm::fgInstance         = NULL;
  std::complex<double>* FemtoYlm::fgYlms = NULL;
  FemtoYlm::FemtoYlm() { InitializeYlms(); }
  ...
  FemtoYlm::FemtoYlm(const FemtoYlm& aYlm) : TObject(aYlm) {
    fgPrefshift = aYlm.fgPrefshift;
    InitializeYlms();
  }

  FemtoYlm& FemtoYlm::operator=(const FemtoYlm& aYlm) {
  ...
  FemtoYlm* FemtoYlm::Instance() {
    if (fgInstance == NULL) { fgInstance = new FemtoYlm(); }
    return fgInstance;
  }

  std::complex<double> FemtoYlm::Ceiphi(double phi) { return std::complex<double>(cos(phi), sin(phi)); }

  double FemtoYlm::Legendre(int ell, int em, double ctheta) {
    // Calculate a single Legendre value
    // *** Warning - NOT optimal - calculated all Plms up to L !!!
    double lbuf[36];
    FemtoYlm::LegendreUpToYlm(ell, ctheta, lbuf);

    return lbuf[fgPlmshift[ell] - abs(em)];
  }

  std::complex<double> FemtoYlm::Ylm(int ell, int m, double theta, double phi) {
  ...
  std::complex<double> FemtoYlm::Ylm(int ell, int m, double x, double y, double z) {
  ...
  double FemtoYlm::ReYlm(int ell, int m, double theta, double phi) { return real(FemtoYlm::Ylm(ell, m, theta, phi)); }

  double FemtoYlm::ImYlm(int ell, int m, double theta, double phi) { return imag(FemtoYlm::Ylm(ell, m, theta, phi)); }

  double FemtoYlm::ReYlm(int ell, int m, double x, double y, double z) { return real(FemtoYlm::Ylm(ell, m, x, y, z)); }

  double FemtoYlm::ImYlm(int ell, int m, double x, double y, double z) { return imag(FemtoYlm::Ylm(ell, m, x, y, z)); }

  void FemtoYlm::InitializeYlms() {
    // Calculate prefactors for fast Ylm calculation

    double oneoversqrtpi = 1.0 / TMath::Sqrt(TMath::Pi());

    if (fgPrefactors != NULL) free(fgPrefactors);
    if (fgPrefshift != NULL) free(fgPrefshift);
    if (fgPlmshift != NULL) free(fgPlmshift);
    ...
    fgPlmshift[4] = 14;
    fgPlmshift[5] = 20;
    fgYlms        = new std::complex<double>[36];
  }

  void FemtoYlm::LegendreUpToYlm(int lmax, double ctheta, double* lbuf) {
  ...
  std::complex<double>* FemtoYlm::YlmUpToL(int lmax, double x, double y, double z) {
  ...
  std::complex<double>* FemtoYlm::YlmUpToL(int lmax, double ctheta, double phi) {
    // Calculate a set of Ylms up to a given l
    // with spherical input
    int lcur = 0;
    double lpol;
    ...
    fgYlms[lcur++] = fgPrefactors[0] * lbuf[0] * std::complex<double>(1, 0);

    for (int il = 1; il <= lmax; il++) {
      // First im = 0
      fgYlms[lcur + il] = fgPrefactors[fgPrefshift[il]] * lbuf[fgPlmshift[il]] * std::complex<double>(1.0, 0.0);
      // Im != 0
      for (int im = 1; im <= il; im++) {
        lpol                   = lbuf[fgPlmshift[il] - im];
        fgYlms[lcur + il - im] = fgPrefactors[fgPrefshift[il] - im] * lpol * std::complex<double>(coss[im - 1], -sins[im - 1]);
        fgYlms[lcur + il + im] = fgPrefactors[fgPrefshift[il] + im] * lpol * std::complex<double>(coss[im - 1], sins[im - 1]);
      }
      lcur += 2 * il + 1;
    }
