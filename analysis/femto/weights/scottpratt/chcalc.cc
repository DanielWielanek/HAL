...
namespace Hal {
  namespace ScottPratt {
    ...
    CCHCalc::CCHCalc() {
      if (INITIALIZED == 0) { InitStaticData(); }
      INITIALIZED += 1;
    };
    ...
    void CCHCalc::InitStaticData() {
      ...
      if (fact == NULL) {
        ...
        fact[0] = fact[1] = 1.0;
        for (m = 2; m <= LMAXFACT; m++)
          fact[m] = fact[m - 1] * double(m);
      }
      if (doublefact == NULL) {
        doublefact    = new double[2 * LMAXFACT + 2];
        doublefact[0] = doublefact[1] = 1.0;
        for (m = 2; m <= 2 * LMAXFACT + 1; m++)
          doublefact[m] = doublefact[m - 2] * double(m);
      }
    ...
    double CCHCalc::Factorial(int n) { return fact[n]; }

    double CCHCalc::DoubleFactorial(int n) { return doublefact[n]; }

    double CCHCalc::Binomial(int lx, int ly) { return binomial[lx + ly][lx]; }
    ...
    double CCHCalc::GetOverlap(int lx, int ly, int lz, int lxprime, int lyprime, int lzprime) {
      int L, Lprime, mx, my;
      L      = lx + ly + lz;
      ...
      if (overlap[lx][ly][lz] == NULL) {
        ...
        for (mx = 0; mx <= L / 2; mx++) {
          ...
          for (my = 0; my <= (L - lxprime) / 2; my++) {
            ...
            overlap[lx][ly][lz][mx][my] = GetOverlap0(lx, ly, lz, lxprime, lyprime, lzprime);
    ...
    double CCHCalc::GetMFromE(int lx, int ly, int lz, double ex, double ey, double ez) {
      return pow(ex, lx) * pow(ey, ly) * pow(ez, lz);
    }

    double CCHCalc::GetMFromThetaPhi(int lx, int ly, int lz, double theta, double phi) {
    ...
    double CCHCalc::GetAFromE(int lx, int ly, int lz, double ex, double ey, double ez) {
      int L, m, mx, my, mz;
    ...
    double CCHCalc::GetOverlap0(int lx, int ly, int lz, int lxprime, int lyprime, int lzprime) {
      ...
      if (L != Lprime) {
        ...
        prefactor =
          4.0 * PI * fact[lx] * fact[ly] * fact[lz] * fact[lxprime] * fact[lyprime] * fact[lzprime] / ((2.0 * double(L) + 1.0));
        if (L > 0) prefactor = prefactor / pow(doublefact[2 * L - 1], 2);
        sum   = 0.0;
        mxmin = mymin = mzmin = 0;
        ...
        for (mx = mxmin; mx <= lx / 2; mx++) {
          for (my = mymin; my <= ly / 2; my++) {
            for (mz = mzmin; mz <= lz / 2; mz++) {
              m = mx + my + mz;
              if (2 * L - 2 * m - 1 > 0)
                factor = doublefact[2 * L - 2 * m - 1] * fact[m];
              else
                factor = 1.0;
